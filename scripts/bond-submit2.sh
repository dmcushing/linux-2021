...
do
	entity_exists - $USERID passwd
	user_in_group - $USERID bonds
	if [[ "$USERID" = "sean" ]];
		user_in_group - $USERID pre80s
		comment - $USERID "Sean Connery"
	fi
	user_in_group - $USERID bonds
	if [[ "$USERID" = "george" ]];
		user_in_group - $USERID pre80s
		comment - $USERID "George Lazenby"
	fi
	user_in_group - $USERID bonds
	if [[ "$USERID" = "roger" ]];
		user_in_group - $USERID pre80s
		comment - $USERID "Roger Moore"
	fi
done
blank_line
...
do
    check_existence - $DIRECTORY d
	check_permissions - $DIRECTORY drwxr-xr-x
done
blank_line
